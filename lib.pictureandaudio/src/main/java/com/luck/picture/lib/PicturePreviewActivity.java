...
public class PicturePreviewActivity extends PictureBaseActivity implements View.OnClickListener, Animation.AnimationListener {
    ...
    public void eventBus(EventBean bean) {
        ...
        if (bean.getCode() == PictureConfig.CLOSE_PREVIEW_FLAG) {
            // 压缩完后关闭预览界面
            dismissDialog();
            finish();
    ...
    private void updateSelector(boolean isRefresh) {
        if (isRefresh) {
            ...
            EventBus.getDefault().post(new EventBean(PictureConfig.UPDATE_FLAG, obj));
        }
    }
    ...
    public void onResult(List<LocalMedia> images) {
       EventBus.getDefault().post(new EventBean(PictureConfig.PREVIEW_DATA_FLAG,new EventEntity(PictureConfig.PREVIEW_DATA_FLAG, images)));
