...
public class ShrinkableTextView extends TextView{
    ...
    //  used to judge if the listener of corresponding to the onclick event of ShrinkableTextView
    ...
    public ShrinkableTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ShrinkableTextView);
        if (a == null) {
            return;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.ShrinkableTextView_etv_MaxLinesOnShrink) {
                mMaxLinesOnShrink = a.getInteger(attr, MAX_LINES_ON_SHRINK);
            }else if (attr == R.styleable.ShrinkableTextView_etv_EllipsisHint){
                mEllipsisHint = a.getString(attr);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToExpandHint) {
                mToExpandHint = a.getString(attr);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToShrinkHint) {
                mToShrinkHint = a.getString(attr);
            }else if (attr == R.styleable.ShrinkableTextView_etv_EnableToggle) {
                mToggleEnable = a.getBoolean(attr, TOGGLE_ENABLE);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToExpandHintShow){
                mShowToExpandHint = a.getBoolean(attr, SHOW_TO_EXPAND_HINT);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToShrinkHintShow){
                mShowToShrinkHint = a.getBoolean(attr, SHOW_TO_SHRINK_HINT);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToExpandHintColor){
                mToExpandHintColor = a.getInteger(attr, TO_EXPAND_HINT_COLOR);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToShrinkHintColor){
                mToShrinkHintColor = a.getInteger(attr, TO_SHRINK_HINT_COLOR);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToExpandHintColorBgPressed){
                mToExpandHintColorBgPressed = a.getInteger(attr, TO_EXPAND_HINT_COLOR_BG_PRESSED);
            }else if (attr == R.styleable.ShrinkableTextView_etv_ToShrinkHintColorBgPressed){
                mToShrinkHintColorBgPressed = a.getInteger(attr, TO_SHRINK_HINT_COLOR_BG_PRESSED);
            }else if (attr == R.styleable.ShrinkableTextView_etv_InitState){
                mCurrState = a.getInteger(attr, STATE_SHRINK);
            }else if (attr == R.styleable.ShrinkableTextView_etv_GapToExpandHint){
                mGapToExpandHint = a.getString(attr);
            }else if (attr == R.styleable.ShrinkableTextView_etv_GapToShrinkHint){
    ...
    /**
     * used in ListView or RecyclerView to update ShrinkableTextView
     * @param text
     *          original text
     * @param futureTextViewWidth
     *          the width of ShrinkableTextView in px unit,
    ...
    /**
     * get the current state of ShrinkableTextView
     * @return
    ...
    public interface OnExpandListener{
        void onExpand(ShrinkableTextView view);
        void onShrink(ShrinkableTextView view);
    }
    ...
    private class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if(hasOnClickListeners()
                    && (getOnClickListener(ShrinkableTextView.this) instanceof ExpandableClickListener)) {
            }else{
                toggle();
            }
        }
