package com.huateng.fm.ui.view.common;
...
import com.huateng.fm.ui.view.FmComboSeekBar;
import com.huateng.fm.ui.view.FmComboSeekBar.Dot;
...
public class CustomDrawable extends Drawable {
	private final FmComboSeekBar mySlider;
	...
	public CustomDrawable(Drawable base, FmComboSeekBar slider, float thumbRadius, List<Dot> dots, int color, int textUnselectedColor,int textSelectedColor,int lineColor, int textSize, boolean isMultiline) {
		...
		selectLinePaint.setColor(lineColor);
	...
	private int verticalPadding=40;
	...
	private void drawText(Canvas canvas, Dot dot, float x, float y) {
		...
		if (mIsMultiline) {
			if ((dot.id % 2) == 0) {
				yres = y - mTextMargin - mDotRadius-verticalPadding;
			} else {
				yres = y + mTextHeight+verticalPadding;
			}
		} else {
	...
	public final int getIntrinsicHeight() {
		if (mIsMultiline) {
			return (int) (selectLinePaint.getStrokeWidth() + mDotRadius + (mTextHeight) * 2  + mTextMargin)+2*verticalPadding;
		} else {
