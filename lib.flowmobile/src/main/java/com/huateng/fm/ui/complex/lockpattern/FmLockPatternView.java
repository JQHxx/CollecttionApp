 ...
 */
public class FmLockPatternView extends View {
	...
	private DisplayMode mPatternDisplayMode = DisplayMode.Animate;
	...
	public FmLockPatternView(Context context) {
		this(context, null);
	}

	public FmLockPatternView(Context context, AttributeSet attrs) {
		super(context, attrs);

		TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.ht_LockPatternView);

		final String aspect = typedArray
				.getString(R.styleable.ht_LockPatternView_aspect);
		...
		mRegularColor = typedArray.getColor(
				R.styleable.ht_LockPatternView_regularColor, mRegularColor);
		mErrorColor = typedArray.getColor(
				R.styleable.ht_LockPatternView_errorColor, mErrorColor);
		mSuccessColor = typedArray.getColor(
				R.styleable.ht_LockPatternView_successColor, mSuccessColor);

		int pathColor = typedArray.getColor(
				R.styleable.ht_LockPatternView_pathColor, mRegularColor);
		...
		int pathWidth = getResources().getDimensionPixelSize(
				R.dimen.lock_pattern_dot_line_width);
		int dotSize = getResources().getDimensionPixelSize(
				R.dimen.lock_pattern_dot_size);
		int dotSizeActivated = getResources().getDimensionPixelSize(
				R.dimen.lock_pattern_dot_size_activated);

		mPathWidth = typedArray.getDimensionPixelOffset(
				R.styleable.ht_LockPatternView_dotLineWidth, pathWidth);
		mPathPaint.setStrokeWidth(mPathWidth);

		mDotSize = typedArray.getDimensionPixelOffset(
				R.styleable.ht_LockPatternView_dotSize, dotSize);
		mDotSizeActivated = typedArray.getDimensionPixelOffset(
				R.styleable.ht_LockPatternView_dotSizeActivated,
				dotSizeActivated);
		...
		valueAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					...
					public void onAnimationUpdate(ValueAnimator animation) {
						float t = Float.parseFloat(animation.getAnimatedValue()
								+ "");
	...
	private void startSizeAnimation(float start, float end, long duration,
		...
		valueAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					@Override
					public void onAnimationUpdate(ValueAnimator animation) {
						state.size = Float.parseFloat(animation
								.getAnimatedValue() + "");
						invalidate();
					}
				});
	...
	private static class SavedState extends BaseSavedState {
		...
		/**
		 * Constructor called from {@link FmLockPatternView#onSaveInstanceState()}
		 */
