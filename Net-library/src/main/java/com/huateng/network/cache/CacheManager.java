package com.huateng.network.cache;
...
public final class CacheManager {
    ...
    private CacheManager() {
        Context context = NetworkConfig.C.getCtx();
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        AppUtils.getAppVersionCode(), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
    ...
    private File getDiskCacheDir(Context context, String uniqueName) {
        ...
        //Log.i("cacheDir",String.format("--------%s--------",cachePath));
